...
table. Every payer is a real signed-in account with their own wallet, so nobody is
...
4. **Open a table** (`/split`): enter subtotal, tip, and either an even split or a
...
6. **People pay their share** at `/pay/{intentId}`, via a direct link to their own
...
**exactly** the total, as floating point would silently lose money at this precision.
...
exercised against the staging API, which needs real credentials.
